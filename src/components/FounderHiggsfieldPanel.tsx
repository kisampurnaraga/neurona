...
export const FounderHiggsfieldPanel: React.FC = () => {
  ...
  const [selectedModel, setSelectedModel] = useState('veo3_1_lite');
  ...
  const handleTestGeneration = async () => {
    ...
    try {
      ...
      const data = await res.json();
      const resolvedUrl = typeof data.videoUrl === 'string'
        ? data.videoUrl
        : (typeof data.assetUrl === 'string'
          ? data.assetUrl
          : (data.videoUrl?.videoUrl || data.videoUrl?.assetUrl || data.videoUrl?.url || data.result?.videoUrl || ''));

      if (data.success && resolvedUrl) {
        setTestVideoResult(resolvedUrl);
            ...
            <div className="p-5 rounded-xl bg-white/5 border border-white/10 space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-3 border-t border-white/5">
                ...
                <div>
                  ...
                  >
                    <option value="veo3_1_lite">Google Veo 3.1 Lite (Fast T2V / I2V - 8 cr)</option>
                    <option value="wan3_0">Wan 3.0 (Character Animation I2V - 8.75 cr)</option>
                    <option value="veo3_1">Google Veo 3.1 Cinematic (Ultra 1080p - 22 cr)</option>
                    <option value="higgsfield-video-pro">Higgsfield Video Pro [Alias → Veo 3.1 Lite] (8 cr)</option>
                    <option value="higgsfield-anim">Higgsfield Anim [Alias → Wan 3.0] (8.75 cr)</option>
                  </select>
                </div>
              </div>
            </div>
                ...
                {testVideoResult && (
                  <div className="mt-4 rounded-xl border border-white/10 bg-black/60 p-3 space-y-2">
                    ...
                    <video
                      ...
                      playsInline
                      loop
                      className="w-full rounded-lg max-h-64 bg-black object-contain border border-white/5"
                    />
                  </div>
                )}
              </div>
