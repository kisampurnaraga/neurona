...
async function run() {
  ...
  const targetUrl = 'http://127.0.0.1:3000/api/fcc/openart/auth/init?origin=https://ais-dev-lhwcbpgrrfalopwm3dt5h2-654788409683.asia-southeast1.run.app';
  
  try {
    const res = await fetch(targetUrl, {
      headers: {
        'x-role': 'founder',
        // Purposely NOT setting x-forwarded-host, just what curl defaults to
      }
    });
